"""
Main CLI entrypoint for the RFP Auto-Responder.

Usage:
    python -m src.main --input samples/sample_rfp.csv --output output/responses.csv

Reads RFP questions from a CSV, processes each through the agent graph,
and writes approved/escalated responses to output CSVs.
"""
...
from src.types import RFPQuestionRow, RFPResponseRow, RFPState
...
def main():
    ...
    if not {"question_id", "question"}.issubset(df.columns):
        raise ValueError("Input CSV must have 'question_id' and 'question' columns")

    questions = [
        RFPQuestionRow(question_id=row["question_id"], question=row["question"])
    ...
    for i, q in enumerate(questions, 1):
        ...
        # Track escalated questions for human review queue
        if status == "escalated":
            escalated_data = {
                "question_id": q.question_id,
                "question": q.question,
    ...
    responses_df = pd.DataFrame([r.model_dump() for r in responses])
    ...
    if escalated_rows:
        ...
        escalated_df = pd.DataFrame(escalated_rows)
        escalated_df.to_csv(review_queue_path, index=False)
